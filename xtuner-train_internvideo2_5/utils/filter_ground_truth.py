"""Filters fine_grained_labels.json to include only ground truths of videos
actually downloaded by load_dataset.py."""
...
def main():
    ...
    if not GROUND_TRUTH_FILE.exists() or not MANIFEST_FILE.exists():
        print(
            '⚠️ Required files missing. Please run DownloadDataset.py first.')
        return
    ...
    filtered = [
        item for item in gt_data if 'video_path' in item
        and Path(item['video_path']).name in downloaded_filenames
    ]
