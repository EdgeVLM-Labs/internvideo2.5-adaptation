#!/usr/bin/env python3
"""Video Augmentation Script for QVED Dataset Uses vidaug library to apply
various augmentation techniques to exercise videos."""
...
AUGMENTATION_OPTIONS = {
    ...
    14: ('Elastic Transformation', va.ElasticTransformation(alpha=10,
                                                            sigma=3)),
}
...
def update_json_files(augmented_videos_info):
    ...
    if OUTPUT_GROUND_TRUTH_FILE.exists():
        with open(OUTPUT_GROUND_TRUTH_FILE) as f:
    ...
    for aug_info in augmented_videos_info:
        ...
        if original_path in fine_grained_labels:
            ...
            fine_grained_labels[augmented_path] = fine_grained_labels[
                original_path].copy()
        ...
        if original_path in manifest:
            ...
            if isinstance(original_value, str):
                ...
                manifest[augmented_path] = original_value.get(
                    'exercise',
                    original_value.get('path',
                                       augmented_path).split('/')[0].replace(
                                           '_', ' '))
            else:
                manifest[augmented_path] = augmented_path.split(
                    '/')[0].replace('_', ' ')
        else:
...
def main():
    ...
    exercise_folders = sorted([
        d for d in BASE_DIR.iterdir()
        if d.is_dir() and (d / '*.mp4' or list(d.glob('*.mp4')))
    ])
    ...
    if folder_input:
        try:
            selected_indices = [
                int(x.strip()) for x in folder_input.split(',')
            ]
            selected_folders = [
                exercise_folders[i - 1] for i in selected_indices
                if 1 <= i <= len(exercise_folders)
            ]
        except (ValueError, IndexError):
            print(
                '❌ Invalid input! Please enter valid comma-separated numbers.')
            return
    else:
    ...
    for folder in selected_folders:
        ...
        try:
            selected_aug_indices = [
                int(x.strip()) for x in aug_input.split(',')
            ]
            selected_augmentors = []
            aug_names = []
            ...
            for idx in selected_aug_indices:
                if idx in AUGMENTATION_OPTIONS:
                    ...
                    print(
                        f'Warning: Invalid augmentation index {idx}, skipping...'
                    )
            ...
            for video_path in videos:
                ...
                for aug_idx in selected_aug_indices:
                    ...
                    # Apply augmentation
                    success = augment_video(video_path, single_aug,
                                            output_path)

                    if success:
                        # Track for JSON update
                        relative_original = str(
                            Path(folder.name) / video_path.name)
                        relative_augmented = str(
                            Path(folder.name) / output_filename)
    ...
    if all_augmented_videos:
        ...
        print('\n' + '=' * 60)
        print(
            f'✓ Augmentation Complete! Created {len(all_augmented_videos)} augmented videos'
        )
        print('=' * 60)
    else:
