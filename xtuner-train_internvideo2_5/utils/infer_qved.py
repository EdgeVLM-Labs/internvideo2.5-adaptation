...
def main():
    parser = argparse.ArgumentParser(
        description='QVED Finetuned Model Inference')
    parser.add_argument(
        '--model_path',
        type=str,
        default='Amshaker/Mobile-VideoGPT-0.5B',
        help=
        'Path to the model (HuggingFace model ID or local checkpoint directory)'
    )
    parser.add_argument(
        '--video_path',
        type=str,
    ...
    if not args.model_path.startswith(
            'Amshaker/') and not args.model_path.startswith(
                'EdgeVLM-Labs/') and not os.path.exists(args.model_path):
