...
if MOTION_FLAGS_FILE.exists():
    with open(MOTION_FLAGS_FILE) as f:
        MOTION_FLAGS = json.load(f)
else:
    raise FileNotFoundError(
        f'Motion flags file not found: {MOTION_FLAGS_FILE}')
...
def copy_video_with_structure(source_file: Path, source_root: Path,
                              destination_root: Path) -> None:
    """Copy a video file to the destination while preserving the folder
    structure."""
...
def analyze_video_quality(video_path: Path, num_frames: int, frame_stride: int,
    ...
    motion_flag = MOTION_FLAGS.get(
        exercise_name,
        False)  # Check if motion analysis is needed for an exercise
    ...
    while samples_collected < num_frames and frame_index < max(
        ...
        if motion_flag:
            ...
            if prev_gray_for_motion is not None:
                ...
                _, thresh = cv2.threshold(diff, MOTION_DIFF_THRESHOLD, 255,
                                          cv2.THRESH_BINARY)
    ...
    if motion_flag and motion_pairs > 0:
        ...
        motion_mean_change_ratio = float(
            np.mean(change_ratios)) if change_ratios else float('nan')
        motion_max_change_ratio = float(
            np.max(change_ratios)) if change_ratios else float('nan')
    ...
    metrics = {
        ...
        height,
        'mean_brightness':
        np.mean(sampled_brightness) if sampled_brightness else float('nan'),
        'sharpness_score':
        np.mean(sampled_sharpness) if sampled_sharpness else float('nan'),
        'motion_flag':
        motion_flag,
        'motion_detected':
        motion_detected,
        'motion_pairs':
        motion_pairs if motion_flag else 0,
        'motion_active_pairs':
        active_count if motion_flag else 0,
        'motion_active_frame_pct':
        motion_active_frame_pct,
        'motion_mean_change_ratio':
        motion_mean_change_ratio,
        'motion_max_change_ratio':
        motion_max_change_ratio,
    }
...
def evaluate_video_acceptance(metrics: dict, issues: list, stats: dict):
    """Check if a video meets the minimum quality thresholds for acceptance and
    ...
    if ('corrupted_file'
            in issues) or np.isnan(brightness) or np.isnan(sharpness):
...
def generate_cleaning_report(overall_exercise_stats,
                             totals,
                             destination_root: Path = None):
    """Generate and optionally save a summary report of the dataset cleaning
    results."""
...
def filter_json_files_for_accepted_videos(destination_root: Path):
    """Filter manifest.json and ground_truth.json to only include videos that
    ...
    if manifest_file.exists():
        ...
        print(
            f'[INFO] Filtered manifest.json: {len(filtered_manifest)}/{len(original_manifest)} videos retained'
        )
    ...
    if ground_truth_file.exists():
        ...
        print(
            f'[INFO] Filtered ground_truth.json: {len(filtered_ground_truth)}/{len(original_ground_truth)} entries retained'
        )


def prompt_replace_dataset(source_root: Path, destination_root: Path) -> None:
    """Ask user whether to replace the original dataset with the cleaned
    dataset."""
    print('\n' + '=' * 50)
    ...
    print('')
    print(
        "  y = Remove original 'dataset' folder and rename 'cleaned_dataset' to 'dataset'"
    )
    ...
    while True:
        ...
        if response in ('', 'n', 'no'):
            ...
            try:
                ...
                if manifest_file.exists():
                    shutil.copy2(manifest_file,
                                 destination_root / 'manifest.json')
                    print(f'[INFO] Preserved: manifest.json')
                if fine_labels_file.exists():
                    shutil.copy2(fine_labels_file,
                                 destination_root / 'fine_grained_labels.json')
                    print(f'[INFO] Preserved: fine_grained_labels.json')
                if ground_truth_file.exists():
                    shutil.copy2(ground_truth_file,
                                 destination_root / 'ground_truth.json')
                    print(f'[INFO] Preserved: ground_truth.json')
                if qved_train_file.exists():
                    shutil.copy2(qved_train_file,
                                 destination_root / 'qved_train.json')
                    print(f'[INFO] Preserved: qved_train.json')
                if qved_val_file.exists():
                    shutil.copy2(qved_val_file,
                                 destination_root / 'qved_val.json')
                    print(f'[INFO] Preserved: qved_val.json')
                if qved_test_file.exists():
                    shutil.copy2(qved_test_file,
                                 destination_root / 'qved_test.json')
                ...
                print(
                    f'  Cleaned dataset still available at: {destination_root}'
                )
            break
        else:
...
def print_exercise_stats(stats: dict) -> None:
    ...
    summary = (f"[SUMMARY] {stats['Exercise']}\n"
...
def clean_dataset(source_root: Path, destination_root: Path):
    ...
    for root, _, files in os.walk(source_root):
        ...
        with tqdm(
            ...
            for file in files:
                ...
                accepted, reasons = evaluate_video_acceptance(
                    metrics, issues, exercise_stats)
                ...
                VIDEO_LOG.append({
                    ...
                    'brightness':
                    None if np.isnan(
                        metrics.get('mean_brightness', float('nan'))) else
                    float(np.round(metrics.get('mean_brightness'), 2)),
                    'sharpness':
                    None if np.isnan(
                        metrics.get('sharpness_score', float('nan'))) else
                    float(np.round(metrics.get('sharpness_score'), 2)),
                    'motion_flag':
                    metrics.get('motion_flag'),
                    'motion_detected':
                    metrics.get('motion_detected'),
                    'motion_pairs':
                    metrics.get('motion_pairs'),
                    'motion_active_pairs':
                    metrics.get('motion_active_pairs'),
                    'motion_active_frame_pct':
                    None if np.isnan(
                        metrics.get('motion_active_frame_pct', float('nan')))
                    else float(
                        np.round(metrics.get('motion_active_frame_pct'), 4)),
                    'motion_mean_change_ratio':
                    None if np.isnan(
                        metrics.get('motion_mean_change_ratio', float('nan')))
                    else float(
                        np.round(metrics.get('motion_mean_change_ratio'), 6)),
                    'motion_max_change_ratio':
                    None if np.isnan(
                        metrics.get('motion_max_change_ratio', float('nan')))
                    else float(
                        np.round(metrics.get('motion_max_change_ratio'), 6)),
                    'decision':
                    decision,
                    'reasons':
                    ', '.join(reasons) if reasons else 'passed_all_checks'
                })

                if accepted:
                    copy_video_with_structure(video_path, source_root,
                                              destination_root)
    ...
    generate_cleaning_report(overall_exercise_stats, totals,
