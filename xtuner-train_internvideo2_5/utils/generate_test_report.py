...
def compute_meteor_score(reference: str, hypothesis: str, metric) -> float:
    ...
    try:
        return metric.compute(
            predictions=[hypothesis], references=[reference])['meteor']
    except:
        return 0.0
...
def compute_llm_accuracy_score(ground_truth: str, prediction: str, tokenizer,
                               model) -> float:
    """Use Mixtral-Instruct as a judge to score prediction against ground
    truth. Returns a score from 1-5 for holistic accuracy and usefulness.
...
def create_excel_report(results: List[Dict],
    ...
    if base_predictions:
        ws.column_dimensions[get_column_letter(
            col_idx)].width = 50  # Base Model Response
        col_idx += 1

    if use_bert:
        ws.column_dimensions[get_column_letter(
            col_idx)].width = 15  # BERT Similarity
        col_idx += 1

    ws.column_dimensions[get_column_letter(col_idx)].width = 15  # METEOR Score
    ws.column_dimensions[get_column_letter(col_idx +
                                           1)].width = 15  # ROUGE-L Score
    ws.column_dimensions[get_column_letter(col_idx +
                                           2)].width = 18  # LLM Accuracy
    ws.column_dimensions[get_column_letter(col_idx +
                                           3)].width = 15  # Throughput
    ws.column_dimensions[get_column_letter(col_idx + 4)].width = 12  # Gen Time
    ws.column_dimensions[get_column_letter(col_idx +
                                           5)].width = 15  # Generated Tokens
    ws.column_dimensions[get_column_letter(col_idx +
                                           6)].width = 15  # Exercise Match
    ...
    exercise_matches = []
    exercise_stats = {
    }  # Track per-exercise statistics: {exercise_name: {'correct': x, 'total': y}}
    ...
    for idx, result in enumerate(results, start=1):
        ...
        if use_bert and bert_model:
            bert_sim = compute_cosine_similarity_bert(ground_truth, prediction,
                                                      bert_model)
            bert_scores.append(bert_sim)

        meteor_sim = compute_meteor_score(ground_truth, prediction,
                                          meteor_metric)
        ...
        if use_llm_judge and llm_tokenizer and llm_model:
            llm_score = compute_llm_accuracy_score(ground_truth, prediction,
        ...
        ws.cell(
            row=row, column=col).value = 'TRUE' if exercise_match else 'FALSE'
        col += 1
    ...
    if use_bert and bert_scores:
        ...
        bert_yellow = sum(1 for s in bert_scores
                          if BERT_YELLOW_THRESHOLD <= s < BERT_GREEN_THRESHOLD)
        bert_red = sum(1 for s in bert_scores if s < BERT_YELLOW_THRESHOLD)
        bert_chart_start_row = len(
            summary_data) + 7  # Row where Green count will be
        summary_data.extend([
            ['BERT Similarity', ''],
            ...
            [
                f'Yellow ({BERT_YELLOW_THRESHOLD}-{BERT_GREEN_THRESHOLD})',
                bert_yellow
            ],
    ...
    if meteor_scores:
        meteor_green = sum(1 for s in meteor_scores
                           if s >= METEOR_GREEN_THRESHOLD)
        meteor_yellow = sum(
            1 for s in meteor_scores
            if METEOR_YELLOW_THRESHOLD <= s < METEOR_GREEN_THRESHOLD)
        meteor_red = sum(1 for s in meteor_scores
                         if s < METEOR_YELLOW_THRESHOLD)
        meteor_chart_start_row = len(
            summary_data) + 7  # Row where Green count will be
        summary_data.extend([
            ['METEOR Score', ''],
            ...
            [
                f'Yellow ({METEOR_YELLOW_THRESHOLD}-{METEOR_GREEN_THRESHOLD})',
                meteor_yellow
            ],
    ...
    if rouge_scores:
        rouge_green = sum(1 for s in rouge_scores
                          if s >= ROUGE_GREEN_THRESHOLD)
        rouge_yellow = sum(
            1 for s in rouge_scores
            if ROUGE_YELLOW_THRESHOLD <= s < ROUGE_GREEN_THRESHOLD)
        ...
        summary_data.extend([
            ...
            [
                f'Yellow ({ROUGE_YELLOW_THRESHOLD}-{ROUGE_GREEN_THRESHOLD})',
                rouge_yellow
            ],
    ...
    if llm_accuracy_scores:
        llm_green = sum(1 for s in llm_accuracy_scores
                        if s >= LLM_GREEN_THRESHOLD)
        llm_yellow = sum(1 for s in llm_accuracy_scores
                         if LLM_YELLOW_THRESHOLD <= s < LLM_GREEN_THRESHOLD)
        llm_red = sum(1 for s in llm_accuracy_scores
                      if s > 0 and s < LLM_YELLOW_THRESHOLD)
        summary_data.extend([
            ['LLM Accuracy (1-5)', ''],
            ...
            [
                f'Yellow ({LLM_YELLOW_THRESHOLD}-{LLM_GREEN_THRESHOLD})',
                llm_yellow
            ],
    ...
    if exercise_matches:
        ...
        exercise_accuracy = (exercise_correct / len(exercise_matches) *
                             100) if exercise_matches else 0
        summary_data.extend([
            ['Exercise Identification', ''],
            ['Overall Correct', exercise_correct],
            ['Overall Incorrect', exercise_incorrect],
            ['Overall Accuracy (%)',
             round(exercise_accuracy, 2)],
            ['', ''],
    ...
    for row_idx, row_data in enumerate(summary_data, start=1):
        for col_idx, value in enumerate(row_data, start=1):
            ...
            if row_idx == 1 or (isinstance(value, str) and value
                                and row_idx > 1 and col_idx == 1):
    ...
    if meteor_scores and meteor_chart_start_row:
        ...
        # Data reference (values)
        meteor_data = Reference(
            summary_ws,
            min_col=2,
            min_row=meteor_chart_start_row,
            max_row=meteor_chart_start_row + 2)
        # Categories reference (labels)
        meteor_cats = Reference(
            summary_ws,
            min_col=1,
            min_row=meteor_chart_start_row,
            max_row=meteor_chart_start_row + 2)
    ...
    if rouge_scores and rouge_chart_start_row:
        ...
        # Data reference (values)
        rouge_data = Reference(
            summary_ws,
            min_col=2,
            min_row=rouge_chart_start_row,
            max_row=rouge_chart_start_row + 2)
        # Categories reference (labels)
        rouge_cats = Reference(
            summary_ws,
            min_col=1,
            min_row=rouge_chart_start_row,
            max_row=rouge_chart_start_row + 2)
...
def main():
    parser = argparse.ArgumentParser(
        description='Generate test evaluation report with similarity scores')
    parser.add_argument(
        '--predictions',
        type=str,
        required=True,
    ...
    if args.include_base_model:
        ...
        if response in ['yes', 'y', 'ok']:
            ...
            try:
                ...
                from utils.base_model_inference import \
                    get_base_model_predictions

                # Load test data
                if not args.test_json:
                    print(
                        '❌ Error: --test-json required for base model inference'
                    )
                    return
