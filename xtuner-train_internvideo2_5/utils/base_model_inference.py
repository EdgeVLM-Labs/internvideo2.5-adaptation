...
def run_base_inference(video_path: str,
                       prompt: str,
                       model,
                       tokenizer,
                       image_processor,
    ...
    try:
        ...
        from mobilevideogpt.mm_utils import (process_video,
                                             tokenizer_image_token)

        # Process video
        video_tensor = process_video(video_path, image_processor,
                                     model.config).to(
...
def get_base_model_predictions(
    ...
    for idx, sample in enumerate(
            tqdm(test_data, desc='Running base model inference')):
        try:
            # Get video path
            ...
            prediction = run_base_inference(
                str(video_path), prompt, model, tokenizer, image_processor,
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Run base model inference on test set')
    parser.add_argument(
        '--test_json', type=str, required=True, help='Path to test JSON file')
    parser.add_argument(
        '--base_model',
        type=str,
    ...
    parser.add_argument(
        '--output',
        type=str,
        default='base_model_predictions.json',
    ...
    parser.add_argument(
        '--limit', type=int, help='Limit number of samples (for testing)')
