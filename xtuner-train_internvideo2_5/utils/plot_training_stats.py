...
def plot_loss(epochs: List[float],
              loss: List[float],
              output_path: str = None,
              eval_epochs: List[float] = None,
              eval_loss: List[float] = None,
              pdf: PdfPages = None):
...
def plot_gradient_norm(epochs: List[float],
...
def plot_learning_rate(epochs: List[float],
                       lr: List[float],
                       output_path: str = None,
                       pdf: PdfPages = None):
...
def plot_combined(epochs: List[float],
    ...
    if len(metrics['loss']) > 10:
        ...
        ma = np.convolve(
            metrics['loss'], np.ones(window) / window, mode='valid')
        ma_epochs = epochs[window - 1:]
        axes[0].plot(
            ma_epochs,
            ma,
...
def plot_eval_metrics(eval_epochs: List[float],
...
def get_summary_text(metrics: Dict[str, List[float]],
    ...
    lines.append(
        f"  Videos per Exercise Class: {dataset_info['max_per_class']}")
    lines.append(
        f"  Number of Exercise Classes: {dataset_info['num_classes']}")
    lines.append(
        f"  Total Videos Downloaded: {dataset_info['total_videos']}\n")

    lines.append('Dataset Split:')
    lines.append(
        f"  Train: {dataset_info['train_count']} samples ({dataset_info['train_ratio']*100:.0f}%)"
    )
    lines.append(
        f"  Val:   {dataset_info['val_count']} samples ({dataset_info['val_ratio']*100:.0f}%)"
    )
    lines.append(
        f"  Test:  {dataset_info['test_count']} samples ({dataset_info['test_ratio']*100:.0f}%)\n"
    )
    ...
    if summary:
        ...
        lines.append(
            f"  Total Runtime: {summary.get('train_runtime', 0):.2f} seconds ({summary.get('train_runtime', 0)/60:.2f} minutes)"
        )
        lines.append(
            f"  Samples/Second: {summary.get('train_samples_per_second', 0):.3f}"
        )
        lines.append(
            f"  Steps/Second: {summary.get('train_steps_per_second', 0):.3f}")
        lines.append(
            f"  Average Train Loss: {summary.get('train_loss', 0):.4f}")
    ...
    lines.append(
        f"  Epoch Range: {min(metrics['epoch']):.2f} - {max(metrics['epoch']):.2f}\n"
    )
    ...
    if 'eval_loss' in metrics and metrics['eval_loss']:
        ...
        lines.append(
            f'  Min Validation Loss: {min(eval_loss):.4f} (epoch {eval_epochs[eval_loss.index(min(eval_loss))]:.2f})'
        )
        lines.append(
            f'  Max Validation Loss: {max(eval_loss):.4f} (epoch {eval_epochs[eval_loss.index(max(eval_loss))]:.2f})'
        )
    ...
    lines.append(
        f"  Min Loss: {min(metrics['loss']):.4f} (epoch {metrics['epoch'][metrics['loss'].index(min(metrics['loss']))]:.2f})"
    )
    lines.append(
        f"  Max Loss: {max(metrics['loss']):.4f} (epoch {metrics['epoch'][metrics['loss'].index(max(metrics['loss']))]:.2f})"
    )
...
def create_summary_page(pdf: PdfPages, metrics: Dict[str, List[float]],
...
def save_summary_stats(metrics: Dict[str, List[float]],
                       summary: Dict[str, float], output_path: str):
...
def main():
    parser = argparse.ArgumentParser(
        description=
        'Plot training statistics from Mobile-VideoGPT finetuning logs')
    parser.add_argument(
        '--log_file', type=str, help='Path to training log file')
    parser.add_argument(
        '--model_name',
        type=str,
        default='qved_finetune_mobilevideogpt_0.5B',
        help='Model name for output directory')
    parser.add_argument(
        '--output_dir',
        type=str,
        help='Custom output directory (overrides model_name)')
    ...
    with PdfPages(report_path) as pdf:
        ...
        plot_gradient_norm(
            epochs,
            metrics['grad_norm'],
            str(output_dir / 'gradient_norm.png'),
            pdf=pdf)
        plot_learning_rate(
            epochs,
            metrics['learning_rate'],
            str(output_dir / 'learning_rate.png'),
            pdf=pdf)
        plot_combined(
            epochs,
            metrics,
            str(output_dir / 'combined_metrics.png'),
            eval_epochs,
            eval_loss,
            pdf=pdf)
        plot_eval_metrics(
            eval_epochs,
            eval_loss,
            str(output_dir / 'eval_metrics.png'),
            pdf=pdf)

    print(f'✓ Saved training report to: {report_path}')

    # Save statistics summary
    save_summary_stats(metrics, summary,
                       str(output_dir / 'training_summary.txt'))
