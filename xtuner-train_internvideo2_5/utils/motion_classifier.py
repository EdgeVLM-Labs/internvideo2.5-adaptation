...
def analyze_motion(video_path: Path, num_frames: int,
                   frame_stride: int) -> bool:
...
def list_videos_by_exercise(root: Path) -> dict:
    ...
    for dirpath, _, filenames in os.walk(root):
        ...
        if mp4_files:
            by_exercise[exercise].extend(
                str(Path(dirpath) / fn) for fn in mp4_files)
    print(f'\nTotal exercises found: {len(by_exercise)}\n')
    return by_exercise


def main() -> None:
    """Run motion classification and write per-exercise CSV and JSON
    outputs."""
    ...
    for ex, video_path in tqdm(
            all_items, desc='Analyzing videos', unit='video'):
        try:
    ...
    summary_rows = [('exercise', 'motion_videos', 'no_motion_videos',
                     'total_videos', 'motion_fraction', 'is_motion_exercise')]
    exercise_flags = {}
    ...
    for ex in sorted(per_ex_counts.keys()):
        ...
        summary_rows.append(
            (ex, m, nm, total, f'{motion_fraction:.2f}', is_motion))
        print(
            f'{ex:20s} motion={m:<3} non_motion={nm:<3} fraction={motion_fraction:.2f} is_motion={is_motion}'
        )
