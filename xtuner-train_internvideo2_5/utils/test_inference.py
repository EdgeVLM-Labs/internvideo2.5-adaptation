...
def load_model(pretrained_path: str,
    ...
    if is_lora_checkpoint:
        ...
        tokenizer = AutoTokenizer.from_pretrained(
            pretrained_path, use_fast=False)
        model = AutoModelForCausalLM.from_pretrained(
...
def run_inference(model,
    ...
    """Runs inference on the given video file and returns prediction with
    throughput metrics."""
    import time
    ...
    with torch.inference_mode():
        ...
        output_ids = model.generate(
            ...
            context_images=torch.stack(context_frames,
                                       dim=0).half().to(device),
...
def warmup_gpu(model,
    ...
    for video_path in warmup_videos[:3]:  # Use up to 3 videos for warmup
        ...
        try:
            ...
            _ = run_inference(model, tokenizer, video_path, prompt, device,
                              max_new_tokens)
...
def main():
    parser = argparse.ArgumentParser(
        description='Run inference on QVED test set')
    parser.add_argument(
        '--model_path',
        type=str,
        required=True,
    ...
    parser.add_argument(
        '--test_json',
        type=str,
        default='dataset/qved_test.json',
    ...
    model, tokenizer = load_model(
        args.model_path, device=args.device, base_model=args.base_model)
    ...
    if warmup_dir.exists() and args.device == 'cuda':
        ...
        if warmup_videos:
            warmup_gpu(model, tokenizer, warmup_videos, args.device,
                       args.max_new_tokens)
    ...
    for item in tqdm(test_data, desc='Processing videos'):
        ...
        try:
            # Run inference
            prediction, metrics = run_inference(model, tokenizer, video_path,
                                                prompt, args.device,
                                                args.max_new_tokens)
            ...
            results.append({
                ...
                'tokens_per_second':
                round(metrics['tokens_per_second'], 2),
                'status':
                'success'
            })
