...
class InternVLChatModel(PreTrainedModel):
    ...
    _no_split_modules = [
        'InternVisionModel', 'LlamaDecoderLayer', 'Phi3DecoderLayer'
    ]
    ...
    def forward(
        ...
        if sp_size > 1:
            ...
            if split_input_ids:
                ...
                input_embeds = all_gather(
                    input_embeds_rank_pre, group=sp_group)
                # torch.cuda.synchronize()
            ...
            if split_pixel_values:
                ...
                if orig_img_batch % sp_size != 0:  # 确保能均匀切
                    max_inputs_len = math.ceil(
                        orig_img_batch / sp_size) * sp_size
        ...
        if labels is not None:
            ...
            if not use_custom_loss:
                ...
                if sp_size > 1:
                    # sp 间均衡
                    loss = rescale_sp_loss(
                        loss, shift_labels, sp_group=sp_group)
            else:
                ...
                if cumulative_lengths is not None:
                    # packing 模式
                    num_tokens = cumulative_lengths[
                        1:] - cumulative_lengths[:-1]
                    num_tokens = num_tokens.squeeze(dim=0).cpu().tolist()
                    if num_tokens[-1] == 0:  # 可能有 pading
                        num_tokens = num_tokens[:-1]
                    if sp_size > 1:
                        # 序列维度切分,有点特殊，只对 labels 进行偏移,然后再切分
                        logits_rank_pre = logits.view(
                            -1,
                            self.language_model.config.vocab_size).contiguous(
                            )

                        labels = labels.view(-1).contiguous()
                        shift_labels = labels[
                            1:]  # 关键: shift 然后在补 -100，解决完整序列被切断带来的影响
                        shift_labels = torch.cat(
                            [shift_labels,
                             shift_labels.new_ones(1) * -100],
                            dim=0)
                        shift_labels_rank_pre = split_for_sequence_parallel(
                            shift_labels, dim=0, sp_group=sp_group)

                        loss_fc = nn.CrossEntropyLoss(reduction='none')
                        loss_rank_pre = loss_fc(logits_rank_pre,
                                                shift_labels_rank_pre)
                        ...
                        if non_zero_loss_len == 0:
                            ...
                            loss = torch.stack(
                                loss_list).sum() / non_zero_loss_len
                    else:
